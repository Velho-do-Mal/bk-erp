...
def _item_material_to_dict(item: ItemMaterial) -> dict:
    return {
        ...
        "material_codigo_bk": item.material.codigo_bk,
        "material_codigo_cliente": item.material.codigo_cliente,
...
def dashboard(request):
    ...
    try:
        ...
        return JsonResponse({"ok": True, "itens_material": itens_material, "itens_servico": itens_servico, "totais": totais}, encoder=DjangoJSONEncoder)
...
def listar_materiais(request):
    ...
    if termo:
        queryset = queryset.filter(
            Q(descricao__icontains=termo) | Q(codigo_cliente__icontains=termo) | Q(codigo_bk__icontains=termo)
        )
...
def salvar_obra(request):
    ...
    return JsonResponse({
        ...
        "obra": {"id": obra.id, "nome": obra.nome, "cliente_id": obra.cliente_id,
...
def salvar_item_material(request):
    ...
    if not orcamento_id:
        return JsonResponse({"ok": False, "erro": "orcamento_id obrigatorio."}, status=400)
    if not material_id:
        return JsonResponse({"ok": False, "erro": "material_id obrigatorio."}, status=400)
    ...
    item = ItemMaterial.objects.create(
        orcamento=orcamento, material=material, quantidade=quantidade, valor_unitario=valor_unitario,
    )
...
def excluir_item_material(request):
    ...
    if not item_id:
        return JsonResponse({"ok": False, "erro": "id obrigatorio."}, status=400)
...
def salvar_item_servico(request):
    ...
    if not orcamento_id:
        return JsonResponse({"ok": False, "erro": "orcamento_id obrigatorio."}, status=400)
    if not servico_id:
        return JsonResponse({"ok": False, "erro": "servico_id obrigatorio."}, status=400)
    orcamento = get_object_or_404(Orcamento, id=orcamento_id)
    ...
    item = ItemServico.objects.create(
        orcamento=orcamento, servico=servico, quantidade=quantidade, valor_unitario=valor_unitario,
    )
...
def exportar_excel(request, orcamento_id):
    orcamento = get_object_or_404(
        Orcamento.objects.select_related("obra", "obra__cliente", "obra__projeto"), id=orcamento_id
    )
    ...
    # Agrupar materiais somando quantidades e valor total
    ...
    for item in orcamento.itens_material.select_related("material").all():
        ...
        if key in mapa_mat:
            ...
            mapa_mat[key] = {
                "codigo_bk": item.material.codigo_bk,
                "codigo_cliente": item.material.codigo_cliente,
    ...
    for m in mapa_mat.values():
        writer.writerow([m["codigo_bk"], m["codigo_cliente"], m["descricao"], m["unidade"], m["quantidade"], m["valor_unitario"], m["valor_total"]])

    # Agrupar servicos somando quantidades e valor total
    ...
    for s in mapa_svc.values():
        writer.writerow([s["nome"], s["unidade"], s["quantidade"], s["valor_unitario"], s["valor_total"]])
