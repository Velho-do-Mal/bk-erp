# Generated by Django 5.1.4 on 2026-03-03 22:58

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ("cadastros", "0001_initial"),
    ]

    operations = [
        migrations.CreateModel(
            name="Conta",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("nome", models.CharField(max_length=200)),
                ("banco", models.CharField(blank=True, max_length=100)),
                (
                    "saldo_inicial",
                    models.DecimalField(decimal_places=2, default=0, max_digits=14),
                ),
                ("ativa", models.BooleanField(default=True)),
                ("observacoes", models.TextField(blank=True)),
            ],
            options={
                "verbose_name": "Conta",
                "ordering": ["nome"],
            },
        ),
        migrations.CreateModel(
            name="Categoria",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("nome", models.CharField(max_length=200)),
                (
                    "tipo",
                    models.CharField(
                        choices=[
                            ("entrada", "Entrada"),
                            ("saida", "Saída"),
                            ("ambos", "Ambos"),
                        ],
                        default="ambos",
                        max_length=10,
                    ),
                ),
                ("observacoes", models.TextField(blank=True)),
                (
                    "pai",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        related_name="subcategorias",
                        to="financeiro.categoria",
                    ),
                ),
            ],
            options={
                "verbose_name": "Categoria",
                "ordering": ["nome"],
            },
        ),
        migrations.CreateModel(
            name="Transacao",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("descricao", models.CharField(max_length=300)),
                (
                    "tipo",
                    models.CharField(
                        choices=[("entrada", "Entrada"), ("saida", "Saída")],
                        max_length=10,
                    ),
                ),
                ("valor", models.DecimalField(decimal_places=2, max_digits=14)),
                ("data_competencia", models.DateField()),
                ("data_vencimento", models.DateField(blank=True, null=True)),
                ("data_pagamento", models.DateField(blank=True, null=True)),
                (
                    "status",
                    models.CharField(
                        choices=[("pendente", "Pendente"), ("realizado", "Realizado")],
                        default="pendente",
                        max_length=10,
                    ),
                ),
                ("referencia", models.CharField(blank=True, max_length=100)),
                ("observacoes", models.TextField(blank=True)),
                (
                    "recorrencia",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("", "Sem recorrência"),
                            ("semanal", "Semanal"),
                            ("quinzenal", "Quinzenal"),
                            ("mensal", "Mensal"),
                            ("bimestral", "Bimestral"),
                            ("trimestral", "Trimestral"),
                            ("semestral", "Semestral"),
                            ("anual", "Anual"),
                        ],
                        default="",
                        max_length=15,
                    ),
                ),
                ("recorrencia_grupo", models.CharField(blank=True, max_length=50)),
                ("anexo_nome", models.CharField(blank=True, max_length=300)),
                ("anexo_tipo", models.CharField(blank=True, max_length=100)),
                ("anexo_dados", models.BinaryField(blank=True, null=True)),
                ("criado_em", models.DateTimeField(auto_now_add=True)),
                (
                    "categoria",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="financeiro.categoria",
                    ),
                ),
                (
                    "centro_custo",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="cadastros.centrosdecusto",
                    ),
                ),
                (
                    "cliente",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="cadastros.cliente",
                    ),
                ),
                (
                    "conta",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="financeiro.conta",
                    ),
                ),
                (
                    "fornecedor",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        to="cadastros.fornecedor",
                    ),
                ),
            ],
            options={
                "verbose_name": "Transação",
                "ordering": ["-data_competencia", "-id"],
            },
        ),
        migrations.CreateModel(
            name="Orcamento",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("ano", models.IntegerField()),
                ("mes", models.IntegerField()),
                (
                    "valor",
                    models.DecimalField(decimal_places=2, default=0, max_digits=14),
                ),
                (
                    "categoria",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        to="financeiro.categoria",
                    ),
                ),
            ],
            options={
                "verbose_name": "Orçamento",
                "ordering": ["ano", "mes"],
                "unique_together": {("categoria", "ano", "mes")},
            },
        ),
    ]
