...
def _calcular_acumulados(boletim: BoletimMedicao, ate_periodo_id: int | None = None) -> dict:
    ...
    if ate_periodo_id:
        try:
            p_ref = PeriodoMedicao.objects.get(id=ate_periodo_id)
...
def detalhe(request, bm_id):
    ...
    # Medições e acumulados do período ativo
...
def api_salvar_boletim(request):
    ...
    bm = get_object_or_404(BoletimMedicao, id=bm_id) if bm_id else BoletimMedicao()
    criada = not bm_id
...
def api_excluir_boletim(request):
    data = _req_data(request)
    bm = get_object_or_404(BoletimMedicao, id=data.get("id"))
    bm.delete()
...
def api_salvar_item(request, bm_id):
    ...
    bm = get_object_or_404(BoletimMedicao, id=bm_id)
    data = _req_data(request)
    item_id = data.get("id")
    ...
    if servico_id:
        try:
            servico = ProdutoServico.objects.get(id=servico_id)
    ...
    item = get_object_or_404(ItemContrato, id=item_id, boletim=bm) if item_id else ItemContrato(boletim=bm)
...
def api_excluir_item(request, bm_id):
    ...
    item.delete()
    return JsonResponse({"ok": True, "mensagem": "Item excluído.", "total_contrato": float(bm.total_contrato)})
...
def api_criar_periodo(request, bm_id):
    ...
    # Cria entradas zeradas para todos os itens do contrato
    for item in bm.itens.all():
        MedicaoItem.objects.get_or_create(periodo=periodo, item=item, defaults={"quantidade_medida": Decimal("0")})
...
def api_salvar_medicao(request, bm_id, periodo_id):
    """Salva as quantidades medidas de um período (recebe lista de {item_id, quantidade})."""
    bm = get_object_or_404(BoletimMedicao, id=bm_id)
    ...
    # Recalcula e retorna dados atualizados
    medicoes_dict = {m.item_id: m.quantidade_medida or Decimal("0")
                     for m in periodo.medicoes.all()}
...
def api_dados_consolidado(request, bm_id):
    """Retorna dados consolidados para o relatório (todas as BMs)."""
    boletim = get_object_or_404(BoletimMedicao, id=bm_id)
...
def api_autocomplete_servico(request):
    ...
    qs = ProdutoServico.objects.filter(ativo=True)
    if termo:
