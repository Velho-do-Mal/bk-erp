...
def busca_global(request):
    ...
    # Cada seção só roda se o usuário tem acesso ao módulo correspondente
    # (mesma regra usada pra bloquear a URL direta do módulo — antes a busca
    # global vazava resultado de módulos que o usuário não pode acessar,
    # porque só filtrava por empresa, nunca por permissão de módulo).
    user = request.user

    # --- Clientes / Fornecedores ---
    if user.tem_modulo('cadastros'):
        try:
